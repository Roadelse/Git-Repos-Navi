...
#@ Introduction
###############################################################################
# This scripts aims to deploy the Git-Repos-Navi in system                    #
...
# ----------------------------------------------------------------------------#
#                                                                             #
# 2024-04-25  Roadelse  Initialized                                           #
###############################################################################
...
if [[ $utest == 0 ]]; then
    if [[ -d "$binarypath" ]]; then
        ln -sf $scriptDir/../src/gits $binarypath
        success "Succeed to link binaries in $binarypath"
        exit 0
    fi

    mkdir -p $scriptDir/export/bin
    ln -sf $scriptDir/../src/gits $scriptDir/export/bin
    text_setenv="# >>>>>>>>>>>>>>>>>>>>>>>>>>> [$proj]
